...
export default class Dashboard extends React.Component {
  ...
  componentDidMount = () => {
    if (this.state.lazadaCode && !this.state.lazada) {
      axios({
        method: 'post',
        url: 'https://marketbucketserver.herokuapp.com/api/v1/marketplaces/authorize/lazada',
        headers: {
      ...
      axios({
        method: 'post',
        url: 'https://marketbucketserver.herokuapp.com/api/v1/marketplaces/authorize/shopee',
        headers: {
          'content-type': 'application/json',
          'authorization': `Bearer ${localStorage.jwt}`
        },
